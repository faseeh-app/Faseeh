...
const video = ref({
  ...
  description: 'Learn professional English vocabulary and expressions commonly used in business meetings and corporate environments.',
  ...
  setInterval(() => {
    if (video.value.isPlaying) {
      ...
      const current = subtitles.value.find(sub =>
        video.value.currentTime >= sub.startTime &&
        video.value.currentTime <= sub.endTime
      )
...
<template>
  <div class="video-player">
    ...
    <div
      ...
      :class="{ 'video-player__main--sidebar-open': isSidebarOpen }"
    >
            ...
            <div class="video-player__video-placeholder">
              ...
              <div class="video-player__subtitles-overlay">
                <div
                  ...
                  class="video-player__current-subtitle"
                >
...
.video-player__video-placeholder:focus-within .video-player__video-overlay { /* Show on focus for accessibility */
  opacity: 1;
}
...
/* Responsive design */
@media (max-width: 1024px) { /* Adjust breakpoint for stats wrapping earlier */
  .video-player__stats {
